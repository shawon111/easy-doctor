...
export default function AppointmentWhatsappTwo({ content = {} , isDemo = false, clinics=[]}) {
    ...
    <section className="py-[120px] px-[20px] md:px-[64px]" id="whatsapp">
      <div className="bg-white border border-[#cbd5e1] rounded-[16px] overflow-hidden grid grid-cols-1 lg:grid-cols-2 shadow-sm">
        <div className="p-12 md:p-16 space-y-8">
          ...
          <h2 className="apttwo-headline-md text-[#0f172a] leading-tight">{content.heading}</h2>
          <p className="apttwo-body-lg text-[#475569] leading-relaxed">{content.body}</p>

          <div className="space-y-4">
            {features.map((feature) => (
              <div key={feature.title} className="flex items-start gap-4">
                <span className="apttwo-material-symbols text-[#2563eb]">{feature.icon}</span>
                <div>
                  <h4 className="font-bold text-[#0f172a]">{feature.title}</h4>
                  {feature.detail ? (
                    <p className="text-sm text-[#475569]">{feature.detail}</p>
                  ) : null}
                </div>
              </div>
            ))}
