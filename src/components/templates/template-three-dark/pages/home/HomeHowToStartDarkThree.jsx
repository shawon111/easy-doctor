...
export default function HomeHowToStartDarkThree() {
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {STEPS.map((step) => (
            <div key={step.title} className="relative text-center">
              <div
                className={`w-20 h-20 bg-surface-container rounded-full mx-auto flex items-center justify-center mb-6 z-10 relative ${
                  step.active ? "border border-primary" : "border border-primary/20"
                }`}
              >
                <span className="material-symbols-outlined text-primary text-3xl">{step.icon}</span>
              </div>
              <h4 className="font-headline-md text-[20px] mb-2">{step.title}</h4>
