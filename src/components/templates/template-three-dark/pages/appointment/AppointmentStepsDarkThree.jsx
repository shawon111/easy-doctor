...
export default function AppointmentStepsDarkThree() {
  return (
    <section className="grid grid-cols-1 md:grid-cols-3 gap-gutter mb-24">
      {STEPS.map((step) => (
        <div key={step.title} className="glass-card p-8 rounded-xl relative overflow-hidden group glow-hover">
          <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <span className="font-display-lg text-[120px] font-extrabold italic">{step.number}</span>
          </div>
