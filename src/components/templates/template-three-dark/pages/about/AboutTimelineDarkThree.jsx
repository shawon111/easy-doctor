...
export default function AboutTimelineDarkThree() {
  return (
    <section className="py-32 overflow-hidden">
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        <div className="text-center mb-20">
          <h2 className="font-headline-lg text-headline-lg text-on-surface">Career Milestones</h2>
          <p className="text-on-surface-variant mt-4">Defining the future of medicine through historical excellence.</p>
        </div>

        <div className="relative">
          <div className="absolute left-1/2 transform -translate-x-1/2 w-0.5 h-full timeline-gradient opacity-30 hidden md:block" />
          ...
          <div className="space-y-24">
            {MILESTONES.map((milestone) => (
              ...
              >
                ...
                <div className="md:w-[45%] glass-card p-8 rounded-2xl md:hidden mb-6">
                  <span className={`font-label-sm ${milestone.periodClass} mb-2 block`}>{milestone.period}</span>
                  <h3 className="font-headline-md text-on-surface">{milestone.title}</h3>
                  <p className="text-on-surface-variant mt-2">{milestone.mobileDescription}</p>
                </div>

                <div className="md:w-[45%] rounded-2xl overflow-hidden border border-outline-variant/20 hidden md:block">
