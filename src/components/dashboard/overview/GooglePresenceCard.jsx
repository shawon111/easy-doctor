...
export async function GooglePresenceCard() {
  const user = await requireUser();
  const { profileCompleted } = user;
  let profileCompleteScore = 95;
  if(profileCompleted){
  ...
  return (
    <div className="xl:col-span-2 flex flex-col justify-between rounded-2xl border border-border bg-card p-4 shadow-[0px_4px_12px_rgba(0,0,0,0.03)] sm:p-6">
      ...
      <div>
        ...
        <div className="mb-5 rounded-xl border border-border bg-muted/40 p-4 sm:mb-6 sm:p-5">
          <div className="mb-2 flex items-end justify-between">
            ...
            <span className="text-lg font-semibold text-primary">{profileCompleteScore}%</span>
          </div>
          <Progress value={profileCompleteScore} className="mb-2 h-2.5 bg-border" />
          <p className="text-sm text-muted-foreground">
            {
              profileCompleted ? "Your google presence profile is complete!" : "Please update google business verification to fullfill google presence"
            }
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-stretch gap-3 rounded-xl border border-sky-200/50 bg-sky-50/60 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link href="/dashboard/google-presence" size="sm" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 sm:w-auto btn rounded-3xl text-sm px-4 py-2">
          Improve SEO
        </Link>
      </div>
    </div>
